...
class ArticleController extends Controller
{
    ...
    {
        $article = Article::create($request->only(['title', 'contents', 'topic_id']));
    ...
    {
        $article->update($request->only(['title', 'contents', 'topic_id', 'user_id']));
        $article->tags()->sync($request->tags);
        $article->save();

        return back()->withSuccess('Article successfully updated !');
    }
